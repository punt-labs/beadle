...
argument-hint: "[list | add <name> <email> | remove <name> | find <name>]"
...
If arguments include name and email after `add` (e.g., `/contacts add Alice alice@example.com`),
use them directly. Otherwise prompt for:

1. Name (required, unique)
2. Email (required)
3. Aliases (optional, comma-separated)
