...
below the Call to Action describes Beadle's target end state --- a signed,
...
headline is named for --- is not yet built at all. The Feature Appendix
states the exact split.
...
\begin{faqpair}{What is your next step to validate your vision?}\label{faq:next-step}
  ...
  five releases since, through \texttt{v0.16.5}. Building has not been
  the bottleneck.
...
\begin{faqpair}{What are the major technical risks?}\label{faq:tech-risks}
  \begin{enumerate}[nosep]
    ...
    \item \textbf{GPG key management complexity.} GPG tooling is notoriously
      ...
      Signing already uses an isolated {\small\texttt{GNUPGHOME}} per
      operation --- the user's own GPG key, not a Beadle-generated one ---
      configured via a {\small\texttt{gpg\_signer}} field the install flow
      already prompts for. The user never runs \texttt{gpg} directly today,
      for the reason this FAQ originally proposed:
      {\small\texttt{beadle-email install}} handles it.

      \textit{What's still vision:} dedicated {\small\texttt{beadle
      init}}/{\small\texttt{sign}}/{\small\texttt{verify}} subcommands, and
      --- the actual differentiator this document is named for --- signature
      \textit{enforcement}. The daemon's command-signature verifier is a
      hard-coded no-op today; nothing currently rejects an unsigned or
      tampered command file. Building that enforcement, not more onboarding
      UX, is the higher-priority gap (see \faqref{faq:next-step}).
...
\begin{faqpair}{What is the estimated development timeline?}\label{faq:timeline}
  ...
  \begin{enumerate}[nosep]
    \item \textbf{Foundation (shipped, distributed)} --- MCP email server
      with 23 tools, four-level PGP trust model, per-contact rwx
    ...
    \item \textbf{Daemon + Pipeline v1 (built, not distributed)} ---
      ...
      adversarial isolation. Tested and e2e verified against a real
      mailbox, but \texttt{make dist} and \texttt{install.sh} do not build
      or ship it.
    ...
    \item \textbf{Future (not built)} --- GPG command-signing enforcement
      (currently a hard-coded no-op), the {\small\texttt{beadle init}}
      onboarding flow, daemon distribution, an LLM planner (replace regex
      with Claude decomposition), multi-channel triggers (biff chat,
      webhooks), an internal scheduler, key rotation management.
  \end{enumerate}
...
Feasibility & \textcolor{RiskAmber}{Medium} &
  ...
  entirely unwritten --- rated Low, this risk hid that the hardest
  remaining work is distribution and onboarding, not pipeline mechanics. \\
Viability & \textcolor{RiskAmber}{Medium} &
  ...
  for has any pull. The 6-month-post-release clock is also undated in
  this document and may already have run out (see \faqref{faq:pnl} and
  \faqref{faq:metrics} for both problems). \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  \featureitem{MCP email server}{23 tools --- list, search, read, send,
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  \featureitem{GPG command signing enforcement}{verify GPG signatures on
    ...
    files. Currently a hard-coded no-op --- this is the mechanism the
    document's headline is named for, and it does not exist
    yet}\label{feat:signing}
