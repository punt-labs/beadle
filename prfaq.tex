...
\begin{faqpair}{What is the estimated development timeline?}\label{faq:timeline}
  ...
  \begin{enumerate}[nosep]
    \item \textbf{Foundation} --- Identity config (UC1), single command
      execution, GPG signing, audit log, CLI interface. This is the minimum
      viable trust loop: sign a command, execute it locally, get a
      cryptographically verified result. A proof-of-concept already validates
      GPG signing, single-command execution via Claude CLI, and Proton Bridge
      email delivery, so this phase starts with reduced unknowns.
    \item \textbf{Email} --- Proton Bridge integration (UC2),
      email-triggered execution, result reporting. Adds remote triggering
      without giving up the local trust model.
    \item \textbf{Pipelines} --- Multi-command pipelines (UC3), two-level
      try-catch, schema validation between stages. Generalizes the
      single-command model into composable workflows.
    \item \textbf{Scheduler} --- Internal cron (UC4), third-party email
      handling (UC5), key rotation (UC7, UC8). Convenience and operational
      hygiene; not part of the core value proposition.
  \end{enumerate}

  \textbf{Cut order:} If scope compresses, Scheduler is cut first ---
  it is operational convenience, not core value. Pipelines can be
  deferred to a fast follow-up if Foundation and Email prove the trust
  model. The kill switch is not timeline-based but adoption-based:
...
\begin{faqpair}{What does the P\&L look like at steady state?}\label{faq:pnl}
  ...
  than 10 weekly PyPI installs, the project enters archival mode (security
