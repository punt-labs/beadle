...
\begin{faqpair}{What are the major technical risks?}\label{faq:tech-risks}
  \begin{enumerate}[nosep]
    ...
    \item \textbf{GPG key management complexity.} GPG tooling is notoriously
      ...
      \textit{enforcement}. The daemon's command-signature verifier
      ({\small\texttt{VerifySignature}}, DES-034) is implemented --- canonical
      signing, isolated verification against a known owner key, GnuPG
      status-fd classification --- but is not yet wired into the command-file
      loader, so nothing currently rejects an unsigned or tampered command
      file. Wiring it in, not more onboarding UX, is the higher-priority gap
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  \featureitem{GPG command signing enforcement}{verify GPG signatures on
    ...
    files. The verifier itself is implemented
    ({\small\texttt{VerifySignature}}, DES-034) --- this is the mechanism
    the document's headline is named for --- but it is not yet wired into
    daemon startup, so nothing rejects an unsigned or tampered file
    today}\label{feat:signing}
