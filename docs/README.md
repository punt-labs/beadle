...
| Doc | What it is |
|-----|-----------|
| [`artifacts/daemon-status.html`](artifacts/daemon-status.html) | Intended design vs. what is actually implemented for `beadle-daemon`, stage by stage, with the documented invariants marked real/partial/absent and an ordered path to closing the gaps. Generated 2026-08-31 against `14aa9ba`; every status was read from source. States plainly that the "tamperproof audit log" invariant has no implementation, that a fresh operator cannot configure the daemon without reading `internal/daemon`, and that the new test tier proves the gates but not their trigger. |
