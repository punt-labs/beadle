...
- **Zero agent authority.** Every action requires a GPG-signed instruction from the owner, with the daemon holding no independent decision-making. `internal/daemon/signature.go`'s `VerifySignature` performs real signature verification (DES-034) — canonical-subset signing, isolated single-key verification, GnuPG status-fd classification — and is wired into the command-file loader (`LoadCommands`/`loadCommand` in `internal/daemon/command.go`, DES-035, `docs/wire-verifysignature.md`, tracked as beadle-iru). `~/.punt-labs/beadle/daemon.json` names the recipe authorizer key via `owner_handle` or `owner_gpg_key_id`; when it does, an unsigned or tampered command file is rejected. When `daemon.json` is absent, unresolvable, or ambiguous, command loading is disabled entirely (beadle-kua) — the daemon starts with zero commands rather than falling back to unsigned execution. There is not yet a `beadle init`/`sign` onboarding flow for a new operator to configure `daemon.json` and sign their own command files (tracked as beadle-7gm); until one exists, a fresh deployment simply runs no daemon-triggered commands at all.
