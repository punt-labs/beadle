...
- **Zero agent authority (target state, being wired now).** Every action is meant to require a GPG-signed instruction from the owner, with the daemon holding no independent decision-making. `internal/daemon/signature.go`'s `VerifySignature` performs real signature verification (DES-034) — canonical-subset signing, isolated single-key verification, GnuPG status-fd classification. Wiring it into the command-file loader (`LoadCommands`/`loadCommand` in `internal/daemon/command.go`) is DES-035, tracked as beadle-iru — see `prfaq.tex`'s Feature Appendix ("Must Do") for the gap.
