...
- **Zero agent authority (enforced when configured).** Every action is meant to require a GPG-signed instruction from the owner, with the daemon holding no independent decision-making. `internal/daemon/signature.go`'s `VerifySignature` performs real signature verification (DES-034) — canonical-subset signing, isolated single-key verification, GnuPG status-fd classification — and is wired into the command-file loader (`LoadCommands`/`loadCommand` in `internal/daemon/command.go`, DES-035, `docs/wire-verifysignature.md`, tracked as beadle-iru). Enforcement is opt-in: it activates once `~/.punt-labs/beadle/daemon.json` names an owner key via `owner_handle` or `owner_gpg_key_id`, at which point an unsigned or tampered command file is rejected, and a `daemon.json` that is present but unresolvable disables command loading entirely rather than falling back to unverified. A daemon with no `daemon.json` still loads command files unverified — the pre-DES-035 behavior — since there is not yet a `beadle init`/`sign` onboarding flow for a new operator to sign their own command files with.
