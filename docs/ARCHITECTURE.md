...
- **Zero agent authority (target state, not yet enforced).** Every action is meant to require a GPG-signed instruction from the owner, with the daemon holding no independent decision-making. `internal/daemon/signature.go`'s `VerifySignature` performs real signature verification (DES-034) — canonical-subset signing, isolated single-key verification, GnuPG status-fd classification — but has no caller yet: the command-file loader (`LoadCommands`/`loadCommand` in `internal/daemon/command.go`) does not invoke it, so nothing yet rejects an unsigned or tampered command file. Wiring it in is tracked as beadle-9zh — see `prfaq.tex`'s Feature Appendix ("Must Do") for the gap.
