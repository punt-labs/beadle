...
\item \textbf{Identity resolution}: determining which email address beadle
  operates as, via a chain of sources (ethos, default file, legacy config).
...
priority (ethos highest, legacy lowest).

\begin{zed}
IdentitySource ::= ethos | defaultFile | legacy
\end{zed}
...
hasEthos = zfalse \land hasDefault = zfalse \land hasLegacy = ztrue \implies
