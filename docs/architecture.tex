...
Credentials are resolved by name.  The keychain maps to
OS-specific stores: macOS Keychain (\texttt{security} CLI) on
Darwin, libsecret (\texttt{secret-tool} CLI) on Linux.  File
credentials live in \texttt{\~{}/.config/beadle/<name>} with mode
600.  Environment variables follow the pattern
\texttt{BEADLE\_<NAME>}.
