...
%
% Long schema predicates: break with `\\` + `\quad` at operator
% boundaries (before \land, \implies, \iff, \lor), never a bare
% newline in the source -- a newline alone is just whitespace to
% LaTeX and produces no line break, which is how five schemas here
% once overflowed the page (worst case 468pt, more than a full
% text-width) while `fuzz -t` and `latexmk` both exited 0 -- neither
% checks whether output fits the page. Do NOT reach for fuzz's own
% `\t1` tab macro as the fix: something in this document's package
% chain reloads the LaTeX kernel's `\t` tie-accent after fuzz.sty
% loads, so `\t1` fails with a `\mathaccent`-in-math-mode error that
% points nowhere near the real cause. `\quad`/`\qquad` work and are
% what every schema below uses.
%
...
(\S~Security Boundaries) --- sends mail via the configured SMTP
transport (Proton Bridge by default), falling back to the Resend API
when that transport is unreachable and no signing is required
(\S~Message Send Path), and verifies PGP signatures using an isolated
GPG keyring.
...
\begin{zed}
  [MSGID, ADDRESS, BODY, HEADER, HEADERVALUE, RAWBYTES, KEYID, \\
  \quad SIGNER, CREDNAME, CREDVALUE, FOLDERNAME]
\end{zed}

$MSGID$ is an IMAP UID (numeric string).  $ADDRESS$ is an RFC~5322
email address.  $BODY$ is the plain-text message body --- and only
the message body.  $HEADER$ is a header field name (e.g.\
\texttt{X-Pm-Origin}); $HEADERVALUE$ is that field's value.
$RAWBYTES$ is the raw RFC~822 message.  $KEYID$ is a GPG key
fingerprint.  $SIGNER$ is a GPG user identity.  $CREDNAME$ is a named
credential (e.g.\ \texttt{imap-password}); $CREDVALUE$ is that
credential's resolved value (a secret).  $FOLDERNAME$ is an IMAP
folder name --- distinct from $CREDNAME$: a folder is not a
credential, and the two must never be interchangeable in the type
system (see \texttt{SelectFolder} below, which previously typed
\texttt{folder?} as $CREDNAME$ in error).

$HEADERVALUE$ and $CREDVALUE$ exist for the same reason $FOLDERNAME$
does: an earlier draft of this document typed a header's value and a
resolved credential's value as $BODY$, purely because both happened
to be opaque strings.  That type-checked --- $BODY$, $HEADERVALUE$,
and $CREDVALUE$ share a carrier --- which is exactly why sharing one
declared type could not catch the confusion.  A credential's secret
value is not an email body, and a header field's value is not one
either; nothing below now lets them be typed as if they were.
...
\begin{schema}{ClassifyTrust}
  ...
  \quad \implies level! = trusted) \\
  \land \\
  (hasPGPSignature = ztrue \land pgpVerified = ztrue \\
  \quad \land \lnot (hasProtonE2E = ztrue \land hasProtonInternal = ztrue) \\
  \quad \implies level! = verified) \\
  \land \\
  (hasPGPSignature = ztrue \land pgpVerified = zfalse \\
  \quad \land \lnot (hasProtonE2E = ztrue \land hasProtonInternal = ztrue) \\
  \quad \implies level! = untrusted) \\
  \land \\
  (hasPGPSignature = zfalse \\
  \quad \land \lnot (hasProtonE2E = ztrue \land hasProtonInternal = ztrue) \\
  \quad \implies level! = unverified)
\end{schema}
...
\begin{schema}{CredentialStore}
  keychainAvail : ZBOOL \\
  keychainValues : CREDNAME \pfun CREDVALUE \\
  fileValues : CREDNAME \pfun CREDVALUE \\
  envValues : CREDNAME \pfun CREDVALUE
\end{schema}
...
\begin{schema}{ResolveCredential}
  ...
  \quad \implies value! = keychainValues(name?)) \\
  \land \\
  ((\lnot (keychainAvail = ztrue \land name? \in \dom keychainValues)) \\
  \quad \land name? \in \dom fileValues \\
  \quad \implies value! = fileValues(name?)) \\
  \land \\
  ((\lnot (keychainAvail = ztrue \land name? \in \dom keychainValues)) \\
  \quad \land name? \notin \dom fileValues \\
  \quad \land name? \in \dom envValues \\
  \quad \implies value! = envValues(name?))
\end{schema}
...
\begin{schema}{Message}
  id : MSGID \\
  headers : HEADER \pfun HEADERVALUE \\
  body : BODY \\
  raw : RAWBYTES \\
  level : TrustLevel
\end{schema}
...
this point and are out of scope for this schema), $body$ is the
plain-text body the MIME-parse step below extracts, $raw$ is the
...
verification, when applicable) has run.  $BODY$ now has a genuine use
here --- until this fix it was declared in \S~Basic Types and never
once instantiated by any schema in this document.
...
(\texttt{internal/pgp/sign.go}, \texttt{internal/pgp/decrypt.go})
because that is
...
\texttt{send\_email}          & \texttt{to}, \texttt{subject}, \texttt{body} & Send via the configured SMTP transport (primary) or Resend (fallback, blocked when signing is required). Recipients may be contact names or aliases. \\
...
\begin{schema}{VerifyCommandSignature}
  ...
  \where
  (fingerprintValid = zfalse \\
  \quad \implies outcome! = csInvalid) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = zfalse \\
  \quad \implies outcome! = csMissing) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = ztrue \\
  \quad \land importedKeyCount \neq 1 \\
  \quad \implies outcome! = csInvalid) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = ztrue \\
  \quad \land importedKeyCount = 1 \\
  \quad \land keyExpired = ztrue \\
  \quad \implies outcome! = csExpired) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = ztrue \\
  \quad \land importedKeyCount = 1 \\
  \quad \land keyExpired = zfalse \\
  \quad \land sawNoPubkey = ztrue \\
  \quad \implies outcome! = csWrongKey) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = ztrue \\
  \quad \land importedKeyCount = 1 \\
  \quad \land keyExpired = zfalse \\
  \quad \land sawNoPubkey = zfalse \\
  \quad \land sawGoodSig = ztrue \\
  \quad \implies outcome! = csGood) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = ztrue \\
  \quad \land importedKeyCount = 1 \\
  \quad \land keyExpired = zfalse \\
  \quad \land sawNoPubkey = zfalse \\
  \quad \land sawGoodSig = zfalse \\
  \quad \land sawExpKeySig = ztrue \\
  \quad \implies outcome! = csExpired) \\
  \land \\
  (fingerprintValid = ztrue \\
  \quad \land hasSignature = ztrue \\
...
\begin{schema}{DaemonGate}
  ...
  \where
  admit! = ztrue \iff \\
  \quad ((trust = trusted) \lor (trust = verified \land keyBound = ztrue)) \\
  \quad \land execute = ztrue \\
  \quad \land cmd? \in commands
\end{schema}
...
\begin{itemize}
  ...
  \item \textbf{cli} --- runs a whitelisted binary directly, or a
    ...
    symlink-followed path (\texttt{filepath.\allowbreak EvalSymlinks})
    must still land inside a whitelisted directory.  Output is capped
    at 1~MB
    (\texttt{io.\allowbreak LimitReader}); anything beyond the cap is
    discarded
