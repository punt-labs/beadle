...
%%----------------------------------------------------------------------
\section{Daemon Architecture (DES-027/028/029)}
%%----------------------------------------------------------------------
...
\begin{enumerate}
  ...
  \item Verifies transport trust --- Proton E2E (trusted) or PGP
    ...
    messages from \texttt{rwx} contacts are rejected.
  \item On pass: creates a pipeline.
\end{enumerate}

\subsection{Pipeline Orchestrator}

A pipeline decomposes an email instruction into a sequence of typed
commands.  Each command is a GPG-signed YAML definition with a typed
argument schema, write-set, budget, timeout, and tool list.

\begin{verbatim}
  Email instruction
    -> Planner (LLM or rule-based)
    -> [cmd1 | cmd2 | ... | auto-reply]
    -> Sequential mission execution
\end{verbatim}

Each stage spawns an ephemeral \texttt{claude -p --bare} session with
per-command MCP configuration and a minimal environment.  Output flows
between stages via \texttt{inputs.previous\_output} in the ethos
mission contract.
...
at each state transition.  On daemon startup, in-flight pipelines
are marked failed and the originator is notified.
...
\begin{enumerate}
  ...
  \item \textbf{Trust gate for execution}: only messages with
    transport trust $\geq verified$ (PGP) or $= trusted$ (Proton E2E)
    AND contact \texttt{x}-bit can trigger daemon pipelines.
