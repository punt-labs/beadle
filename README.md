...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/beadle/fc23865/install.sh | sh
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/beadle/fc23865/install.sh -o install.sh
cat install.sh
sh install.sh
```
...
3. **Handle → ethos identity YAML** — `~/.punt-labs/ethos/identities/<handle>.yaml` supplies name and email; `<handle>.ext/beadle.yaml` supplies the GPG key id. Always global, never a repo-local vendored copy.
